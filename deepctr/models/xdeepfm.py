...
def xDeepFM(linear_feature_columns, dnn_feature_columns, dnn_hidden_units=(256, 256),
    ...
    dnn_logit = tf.keras.layers.Dense(
        11, use_bias=False, kernel_initializer=tf.keras.initializers.glorot_normal(seed))(dnn_output)
    ...
    if len(cin_layer_size) > 0:
        ...
        exFM_logit = tf.keras.layers.Dense(11, kernel_initializer=tf.keras.initializers.glorot_normal(seed))(exFM_out)
        final_logit = add_func([final_logit, exFM_logit])
	
    final_logit=Flatten()(final_logit)
    output =  tf.keras.layers.Dense(11, activation=tf.nn.softmax)(final_logit)
