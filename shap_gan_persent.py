...
data = pd.read_csv('./data_generated/train_hig.csv')
...
train_onehot = pd.get_dummies(df, columns = data.columns.values[0:-1], prefix_sep='_', dummy_na=False, drop_first=False)

train = train_onehot.iloc[:,1:]

y_train = train_onehot.iloc[:,0]
model = xgboost.XGBClassifier(learning_rate=0.1, n_estimators=100)
model.fit(train, y_train)

explainer = shap.TreeExplainer(model)
shap_values = explainer.shap_values(train)
mean_shap_values = shap_values.mean(axis=0)
mean_abs_shap_values = [abs(x) for x in mean_shap_values]

column = train_onehot.columns.values[1:]
column_shap = list(zip(column, mean_abs_shap_values))
column_shap.sort(key=operator.itemgetter(1), reverse=True)
first,end = zip(*column_shap)
print(first)
j = 0
for i in first:
    if '_' in  i:
...
data.to_csv('./data_shap/shap_hig_cut_3_0.3.csv',index=False)
...
data_new.to_csv('./data_shap/shap_hig_gan_3_0.3.csv',index=False)